...
# 
# Compatibility Fixed Version - Fix import issues for different vLLM versions
# Based on original benchmark_moe.py to fix _get_config_dtype_str import error
...
def _get_config_dtype_str_compatible(dtype, use_int8_w8a16=False, use_fp8_w8a8=False):
    """
    Compatible version of config dtype string function
    Fix location changes of _get_config_dtype_str in different vLLM versions
    """
    if use_fp8_w8a8:
        return "fp8_w8a8"
    elif use_int8_w8a16:
...
# Multi-level import attempt with graceful fallback
_get_config_dtype_str = None
import_status = ""

try:
    # Try importing from main location
    from vllm.model_executor.layers.fused_moe.config import _get_config_dtype_str as _original_func
    _get_config_dtype_str = _original_func
    import_status = "Using native _get_config_dtype_str (vllm.model_executor.layers.fused_moe.config)"
except ImportError:
    try:
        # Try importing from alternative location
        from vllm.model_executor.layers.fused_moe import _get_config_dtype_str as _original_func
        _get_config_dtype_str = _original_func
        import_status = "Using native _get_config_dtype_str (vllm.model_executor.layers.fused_moe)"
    except ImportError:
        try:
            # Try importing from other possible location
            from vllm.model_executor.layers.fused_moe.layer import _get_config_dtype_str as _original_func
            _get_config_dtype_str = _original_func
            import_status = "Using native _get_config_dtype_str (vllm.model_executor.layers.fused_moe.layer)"
        except ImportError:
            # Use compatible implementation
            _get_config_dtype_str = _get_config_dtype_str_compatible
            import_status = "Using compatible _get_config_dtype_str (custom implementation)"

print(f"vLLM Compatibility Status: {import_status}")

# Create logger to avoid undefined logger error
logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO)

# Compatibility check for FusedMoEQuantConfig.make parameters
def make_quant_config_compatible(quant_dtype, w1_scale, w2_scale, a1_scale, a2_scale, block_shape=None):
    """
    Compatible wrapper for FusedMoEQuantConfig.make() with different vLLM versions
    """
    # Try different parameter combinations for different vLLM versions
    param_combinations = [
        # Most recent version
        {
        ...
        # Without block_shape, alternative dtype name
        {
            'dtype': quant_dtype,
            'w1_scale': w1_scale,
            'w2_scale': w2_scale,
    ...
    # Filter out None values for each combination
    ...
    raise TypeError(f"Unable to create FusedMoEQuantConfig with any known parameter combination. Last error: {e}")
...
def benchmark_config(
    ...
    def run():
        ...
        # Use compatible wrapper for FusedMoEQuantConfig.make
...
def main(args: argparse.Namespace):
    ...
    print(f"Compatibility Status: {import_status}")
